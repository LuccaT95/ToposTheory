...
class productCategory = category +
  ...
  assumes 
          ...
          catProd3: "E (p1 a) \<longrightarrow> (E a)" and
          catProd4: "E (p2 b) \<longrightarrow> (E b)" and
          catProd2: "typeE a \<and> typeE b \<longrightarrow> product a b (a \<^bold>\<times> b) (p1 (a \<^bold>\<times> b)) (p2 (a \<^bold>\<times> b))" and
          catProd5: "commSquare a (p1 ((dom a) \<^bold>\<times> (dom b))) (a \<^bold>\<times> b) (p1 ((cod a) \<^bold>\<times> (cod b)))" and
          catProd6: "commSquare b (p2 ((dom a) \<^bold>\<times> (dom b))) (a \<^bold>\<times> b) (p2 ((cod a) \<^bold>\<times> (cod b)))"
...
lemma "((dom a) \<^bold>\<times> (dom b)) \<cong> dom (p1 ((dom a) \<^bold>\<times> (dom b)))" 
  by (metis S1 S3 S5 S6 catProd1 catProd2 catProd3)

\<comment> \<open>This is not true for the codomain: The reason is that (dom a) might exist even though (dom a \<^bold>\<times> dom b) does not.\<close>
lemma "(dom a) \<cong> cod (p1 ((dom a) \<^bold>\<times> (dom b)))" oops

lemma "type A" oops

lemma "(typeE a \<and> typeE b) \<longrightarrow> typeE (a \<^bold>\<times> b)" 
  by (metis local.S3 local.S5 local.S6 local.catProd2)
...
class cartesianCategory = productCategory +
  fixes equalizer::"'a \<Rightarrow> 'a \<Rightarrow> 'a" ("\<doteq>") and
        finalObject::"'a" ("\<^bold>1") and
        finalMap::"'a \<Rightarrow> 'a" ("!\<^sub>1")
  assumes
        cartesian1: "f:(dom g)\<rightarrow>(cod g) \<longrightarrow> isEqualizer f g (equalizer f g)" and
        ...
        cartesian5: "E (equalizer f g) \<longrightarrow> f:(dom g)\<rightarrow>(cod g)"
begin
lemma "True" nitpick[satisfy] oops
...
lemma "\<not> (E (equalizer f g)) \<longrightarrow> \<not> (f:(dom g) \<rightarrow> (cod g))" 
...
abbreviation (in productCategory) isExponential:: "'a \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> ('a \<Rightarrow> 'a) \<Rightarrow> bool" ("Exp")
  where "Exp a b c \<epsilon> tp \<equiv> \<epsilon>:(a\<^bold>\<times>c)\<Rightarrow>b \<and> (\<^bold>\<forall>z f. f:(z \<^bold>\<times> a)\<rightarrow>b \<longrightarrow>
                            ((tp f):z\<rightarrow>c) \<and> \<epsilon> \<cdot> ((dom a)\<^bold>\<times>(tp f)) \<simeq> f \<and>
                              (\<^bold>\<forall>\<ff>. \<ff>:z\<rightarrow>c \<and> \<epsilon> \<cdot> ((dom a)\<^bold>\<times>\<ff>) \<simeq> f \<longrightarrow> \<ff> \<simeq> (tp f)))"
...
class exponentialCategory = productCategory +
  ...
  assumes G1: "(typeE a \<and> typeE b) \<longrightarrow> (Exp a b (exp a b) (\<epsilon> a b) tp)" and (*Probably needs to be extended to form functor*)
          G2: "E (exp a b) \<longrightarrow> E a \<and> E b" and
          G3: "E (\<epsilon> a b) \<longrightarrow> E a \<and> E b" and
          G4: "E (tp a) \<longrightarrow> E a"
begin
lemma "True" nitpick[satisfy] oops
end

class topos = cartesianCategory + exponentialCategory +
...
lemma "true:\<^bold>1\<Rightarrow>\<Omega>" (*sledgehammer(S1 S2 S3 S5 S6 T1 cartesian2 cartesian3)*)
  by (smt S2 S3 S6 T1 cartesian2 cartesian3)
